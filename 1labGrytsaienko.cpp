...
void MainWindow::on_VocOperand2_clicked()
{
    QString str = ui -> plainTextEdit ->toPlainText();
    QRegExp rx("([\\w\\']+)[\\s,.;]");
    QStringList list;
    ...
    QString types = "int void string double float iterator List const array foe while do goto";
    QStringList ltypes;
    ltypes = types.split(" ");
    ...
    n2 = list.count();
    ui ->label_5 -> setText(QString::number(list.count()));
...
R = r;
n1 = ress.count();
ui -> label_6 -> setText(QString::number(ress.count()+r));
...
void MainWindow::on_AllOperandN2_clicked()
{
    ...
    for(int i = list.count()-1; i >= 0; --i)
    {
        ...
        N2 = list.count();
        ui -> label_8 -> setText(QString::number(list.count()));
...
ui -> label_9 -> setText(QString::number(s+str.count("if(")));
}
...
{
 m = n1 + n2 + R;
 ui -> label_10 -> setText(QString::number(m));
}
...
{
    N = N1 + N2 + R;
    ui -> label_11 -> setText(QString::number(N));
}

void MainWindow::on_V_clicked()
{
    float q = N * qLn(m)/ qLn(2);
    ui -> label_12 -> setText(QString::number(q));
}
...
void MainWindow::on_CI_clicked()
{
    ...
    N1 = ress.count();
    ui -> label_13 -> setText(QString::number(ress.count()+R));
